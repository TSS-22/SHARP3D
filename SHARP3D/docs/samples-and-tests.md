...
Many files missing some critical (and required by the C3D standard) parameters could be "read" in some special case, and can be by their respective application. We made the choice not to read them as they would create strange expection of files. If you need some of those files to be read, open an issue on Github or a discussion to see what can be done with your use case.
...
The SIG/MIPS files are invalid because the pointer to next parameter is invalid: it is in little endian format instead of big endian as expected. Due to SHARP3D parameter reading logic, this is not recoverable at the moment. If this is a problem for you, let us know by opening an issue or a discussion on our Github. 
...
Four excellent sample C3D data files featuring multiple 3D data points. They use [TRIAL frame count](../c3d_docs/appendix/3d-frame-count/main.md) and didn't format it correclty. The [TRIAL:ACTUAL_START_FIELD](../c3d_docs/parameters/additional/trial/trial-actual_start_field.md) and [TRIAL:ACTUAL_END_FIELD](../c3d_docs/parameters/additional/trial/trial-actual_end_field.md) are formatted with additional, unused and unecessary column. This bad formatting is recoverable recoverable, and outside of this mistake, the files are valid.
...
The [ANALOG:BITS](../c3d_docs/parameters/required/analog/analog-bits.md) is present but empty. This is recoverable.
...
Two sample C3D files from a BTS system with TYPE-1 force plate data. Some formatting issues with the original files have been corrected in a third file included in this collection, so only `type-1.c3d` is actually a valid file. The others, `dyamic.c3d` and `standing.c3d` can't be opened because they lack some critical parameters. They can still be found in the SampleError folder for interested people.
...
#### Sample00 - Missing Parameter

Advanced Realtime Tracking GmbH is missing [POINT:RATE](../c3d_docs/parameters/required/point/point-rate.md). This error is unrecoverable. See Sample00 for more information about this data set.

#### Sample02 - Incorrect pointer formatting

The SIG/MIPS files have the last parameter incorrectly formatted. The pointer to next parameter is formatted in little endian format instead of the expected big endian. This is not recoverable at the moment due to SHARP3D reading logic. 

See [Sample02 for more information](#sample02---c3d-files-for-data-format-testing).

#### Sample06 - Parameter Name Error
...
#### Sample28 - Missing Parameters

The files are unrecoverable. See [Application test suite Sample28](#sample28---sample-gait-with-type-1-force-plates) for more information.
...
#### Sample34 - Missing Parameters

Three sample C3D data files exported from the MEMS based Xsens motion capture system containing 3D data, an IMU motion capture system. The files lack some critical parameters and therefor can't be read
