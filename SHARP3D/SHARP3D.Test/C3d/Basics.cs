...
namespace SHARP3D.Test.C3dTests
{
    public class Basics
    {
        ...
        {
            // First frame point
            if (c3d.Data.Points[0].Point[0,0] != null)
            {
                for (int i = 0; i < 3; i++)
                {
                    Assert.Equal(expectedResults.PointFirst0[i], c3d.Data.Points[0].Point[0, i]);
                }
            }
            else
            {
            ...
            // Last frame point
            if (c3d.Data.Points[0].Point[c3d.Data.Points[0].Point.GetLength(0) - 1, 0] != null)
            {
                for (int i = 0; i < 3; i++)
                {
                    Assert.Equal((double)expectedResults.PointLast0[i], (double)c3d.Data.Points[0].Point[c3d.Data.Points[0].Point.GetLength(0) - 1, i], precision: 2);
                }
            }
            else
            {
