...
The loss of resolution during the conversion of the floating-point values to signed integer values, limited by the [POINT:SCALE factor](../parameters/required/point/point-scale_factor.md), results in loss of data resolution when the results approach the [POINT:SCALE](../parameters/required/point/point-scale_factor.md) value due to bad scaling choices.
...
You must ensure that all [ANALOG:GEN_SCALE](../parameters/required/analog/analog-gen_scale.md) and [ANALOG:SCALE](../parameters/required/analog/analog-scale.md) Parameters are set to values that scale the Analog data in meaningful ways. Thus force plate data
channels will contain [ANALOG:SCALE](../parameters/required/analog/analog-scale.md) values that are consistent with the scaling calculations that are required by the [force plate TYPE](../parameters/required/force_platform/force_platform-type.md) description. Other analog channels that containing data with known scaling, for example strain gauge signals, or torque, velocity, and angle data from a dynamometer system etc., should have [ANALOG:SCALE](../parameters/required/analog/analog-scale.md) values that make sense and are described in the [ANALOG:LABELS](../parameters/required/analog/analog-labels.md) and [ANALOG:DESCRIPTIONS](../parameters/required/analog/analog-descriptions.md) entries.
